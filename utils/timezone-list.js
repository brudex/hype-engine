/**
 * Timezone List Utility
 * Generates a list of timezones grouped by continent
 */
class TimezoneList {
    constructor() {
        this.generalTimezones = ['UTC'];
    }

    /**
     * Get all timezones grouped by continent
     * Uses comprehensive timezone list
     */
    getAllTimezones() {
        // Comprehensive timezone list by continent
        return {
            'Africa': [
     ...
     * @param {string} continent - Continent name (optional)
     * @returns {string} - Formatted timezone name
     */
    formatTimezone(timezone, continent = null) {
        let displayedTimezone = timezone;
        
        if (continent) {
            displayedTimezone = timezone.replace(continent + '/', '');
        }

        // Normalize: Replace underscores and slashes
        displayedTimezone = displayedTimezone
            .replace(/_/g, ' ')
            .replace(/\//g, ' / ');

        // Get offset using Node.js compatible method
        try {
            const now = new Date();
            const utc = new Date(now.toLocaleString('en-US', { timeZone: 'UTC' }));
            const local = new Date(now.toLocaleString('en-US', { timeZone: timezone }));
            const offset = (local - utc) / (1000 * 60); // offset in minutes
            const hours = Math.floor(Math.abs(offset) / 60);
            const minutes = Math.abs(offset) % 60;
            const sign = offset >= 0 ? '+' : '-';
            const offsetStr = `${sign}${String(hours).padStart(2, '0')}:${String(minutes).padStart(2, '0')}`;
            
            return `(GMT${offsetStr}) ${displayedTimezone}`;
        } catch (e) {
    ...
    /**
     * Get grouped timezone list
     * @returns {object} - Timezones grouped by continent
     */
    getGroupedList() {
        const list = {};
        const allTimezones = this.getAllTimezones();

        // Add general timezones
        if (this.generalTimezones.length > 0) {
            list['General'] = {};
            this.generalTimezones.forEach(tz => {
                list['General'][tz] = tz;
            });
        }

        // Add continent timezones
        Object.keys(allTimezones).forEach(continent => {
            const timezones = allTimezones[continent];
            if (timezones && timezones.length > 0) {
                list[continent] = {};
                timezones.forEach(tz => {
                    list[continent][tz] = this.formatTimezone(tz, continent);
                });
            }
        });
    ...
    /**
     * Get flat timezone list
     * @returns {object} - Flat object of timezone code => display name
     */
    getFlatList() {
        const list = {};
        const allTimezones = this.getAllTimezones();
        ...
        Object.keys(allTimezones).forEach(continent => {
            ...
            if (timezones && timezones.length > 0) {
                timezones.forEach(tz => {
                    list[tz] = this.formatTimezone(tz, continent);
                });
            }
        });
