...
async function handleLinkedInLinkGeneration(projectUuid) {
    ...
    const baseUrl = 'https://hypeengine.cachetechs.com';
    const callbackUrl = baseUrl + '/integrations/linkedin/callback?projectUuid=' + projectUuid;
    ...
    try {
        ...
        if (linkResult) {
            ...
            if (!placeholderAccount) {
                ...
                placeholderAccount.accessToken = linkResult.token || (provider === 'linkedin' ? 'pending' : placeholderAccount.accessToken);
        ...
        req.flash('error', 'Connect is not supported for this platform. Supported: Twitter/X, LinkedIn.');
        return res.redirect('/dashboard/error');
    } catch (error) {
